    ...
    def get_legal_moves(self, state : GameState, hand : list[Card]) -> list[Card]:
        ...
        if same_suit and (state.contract == lead_suit or state.contract == "AT"):
            ...
            for c in state.played_moves:
                if state.rules.get_power(c, state.contract) > highest_rank and c.suit == lead_suit: 
                    highest_rank = state.rules.get_power(c, state.contract)
            higher = [c for c in same_suit if state.rules.get_power(c, state.contract) > highest_rank]
            if higher: 
                return higher
            return same_suit 
        elif same_suit:
...
class PlayRuleRaise(PlayRule):
    def get_legal_moves(self, state : GameState, hand : list[Card]) -> list[Card]:
        ...
        (winner, _) = state.rules.get_trick_winner(state)
        if winner == state.rules.get_partner(player) or state.contract == "AT" or state.contract == "NT":
            return hand

        played_trumps = [c for c in state.played_moves if c.suit == state.contract]
        highest_trump = 0
        for c in played_trumps:
            highest_trump = max(state.rules.get_power(c, state.contract), highest_trump)
        trumps = [c for c in hand if c.suit == state.contract and state.rules.get_power(c, state.contract) > highest_trump]
        if trumps:
            return trumps
