...
class BidRLAgent(RLAgent):
    ...
    def get_action(self, state : State, player_idx : int, training : bool = False) -> str:
        legal_bids = state.rules.get_legal_bids(state, BidRules)
        if not legal_bids: 
            return
        ...
        legal_ids = [6 if c == "Pass" else state.rules.CONTRACTS.index(c) for c in legal_bids]
        ...
        for b in legal_ids:
            if b == best_bid_id:
                return "Pass" if b == 6 else state.rules.CONTRACTS[b]
        return legal_bids[0]
